...
窗口顶部一行视图标签：`启动板` / `收纳盒` / `待办` / `浏览`（后者尚未实现）。

- 切换视图：点标签，或 `Ctrl+1..4`，或 `Ctrl+Tab` 循环；上次的视图记在 `ui.txt`
...
- 右键菜单：新建条目 / 新建分组 / 重命名分组 / 删除分组 / 重命名条目 / 删除条目 / 打开所在位置
- 按住条目拖到顶部分组标签上松开即换组
...
- `config.txt` 目前只读不写：程序暂无可配置项，该文件存在只是为了给后续阶段留位置
- 无拼音搜索（子串匹配，中文按码点比较）
...
src/
  ...
  views/            grid（共用网格：布局/命中/渲染/导航）、launcher、box
