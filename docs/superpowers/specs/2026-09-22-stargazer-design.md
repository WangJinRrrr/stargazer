...
呼出后焦点默认落在当前视图的主输入控件。`Esc` 一律隐藏窗口。顶部一行视图标签，可点击或 `Ctrl+Tab` 循环切换
（原先的 `Ctrl+1..4` 已取消：那三个数字键改给收纳盒当盒子切换，见 §11.2）。
...
- **交互**：双击打开；`Del` 删引用；`F2` 改显示名；`Ctrl+C` 复制路径（同时提供 `CF_HDROP` 与 `CF_UNICODETEXT`）；
  `Ctrl+1..9` 直接跳到第 N 个盒子（视图切换换成 `Ctrl+Tab` 后腾出来的数字键）；
  `Ctrl+Shift+D` 一键清理失效项；拖入添加；拖出为 `CF_HDROP`；内部拖拽排序；拖到盒子标签即移动到该盒子。
